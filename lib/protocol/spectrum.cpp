...
void SpectrumTable::Get(T* tx, const K& k, evmc::bytes32& v, size_t& version) {
    Table::Put(k, [&](V& _v) {
        ...
        while (rit != end) {
            ...
            LOG(INFO) << tx->id << "(" << tx << ")" << " read " << KeyHasher()(k) % 1000 << " version " << rit->version << std::endl;
            return;
        }
        version = 0;
        LOG(INFO) << tx->id << "(" << tx << ")" << " read " << KeyHasher()(k) % 1000 << " version 0" << std::endl;
...
void SpectrumTable::Put(T* tx, const K& k, const evmc::bytes32& v) {
    ...
    LOG(INFO) << tx->id << "(" << tx << ")" << " write " << KeyHasher()(k) % 1000 << std::endl;
    Table::Put(k, [&](V& _v) {
        auto rit = _v.entries.rbegin();
        auto end = _v.entries.rend();
        ...
        while (rit != end) {
            ...
            for (auto _tx: rit->readers) {
                LOG(INFO) << KeyHasher()(k) % 1000 << " has read dependency " << "(" << _tx << ")" << std::endl;
                if (_tx->id > tx->id) {
                    LOG(INFO) << tx->id << " abort " << _tx->id << std::endl;
                    _tx->SetWAR(k, tx->id);
                }
            }
            break;
...
void SpectrumTable::RegretGet(T* tx, const K& k, size_t version) {
    LOG(INFO) << "remove read record " << tx->id << "(" << tx << ")" << " from " << KeyHasher()(k) % 1000 << std::endl;
    Table::Put(k, [&](V& _v) {
        ...
        #if !defined(NDEBUG)
        {
            ...
            for (auto vit = _v.entries.begin(); vit != end; ++vit) {
                LOG(INFO) << "spot version " << vit->version << std::endl;
...
void SpectrumTable::RegretPut(T* tx, const K& k) {
    LOG(INFO) << "remove write record " << tx->id << "(" << tx << ")" << " from " << KeyHasher()(k) % 1000 << std::endl;
    Table::Put(k, [&](V& _v) {
        auto vit = _v.entries.begin();
        auto end = _v.entries.end();
        while (vit != end) {
...
void SpectrumTable::ClearGet(T* tx, const K& k, size_t version) {
    ...
    Table::Put(k, [&](V& _v) {
        ...
        while (vit != end) {
            ...
            LOG(INFO) << "remove " << tx->id << "(" << tx << ")" << " from version " << vit->version << std::endl; 
            vit->readers.erase(tx);
            break;
        }
        if (version == 0) {
...
void SpectrumTable::ClearPut(T* tx, const K& k) {
    LOG(INFO) << "remove write record before " << tx->id << "(" << tx << ")" << " from " << KeyHasher()(k) % 1000 << std::endl;
    Table::Put(k, [&](V& _v) {
...
void SpectrumExecutor::Generate() {
    ...
    tx->InstallSetStorageHandler([this](
        ...
        if (tx->HasWAR()) {
            LOG(INFO) << "spectrum tx " << tx->id << " break" << std::endl;
            tx->Break();
        }
        LOG(INFO) << "tx " << tx->id <<
    ...
    LOG(INFO) << "spectrum execute " << tx->id;
    tx->Execute();
...
void SpectrumExecutor::ReExecute() {
    LOG(INFO) << "spectrum re-execute " << tx->id;
    ...
    if (back_to == ~size_t{0}) {
        LOG(INFO) << "tx " << tx->id << " do not have to rollback" << std::endl;
        tx->Execute(); return;
    }
...
void SpectrumExecutor::Finalize() {
    LOG(INFO) << "spectrum finalize " << tx->id;
    ...
    if (!tx->HasWAR()) {
        ...
        statistics.JournalAbort();  // 如果事务最终失败，记录中止
    }
