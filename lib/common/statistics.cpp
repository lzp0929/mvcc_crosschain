...
void Statistics::JournalAbort() {
    count_abort.fetch_add(1, std::memory_order_relaxed);
}

void Statistics::JournalTransaction() {
    count_transaction.fetch_add(1, std::memory_order_relaxed);
}
...
std::string Statistics::Print() {
    ...
    return std::string(fmt::format(
        ...
        count_transaction.load(),
        count_abort.load(),
        count_transaction.load() > 0 ? (double)count_abort.load() / count_transaction.load() * 100.0 : 0.0,
